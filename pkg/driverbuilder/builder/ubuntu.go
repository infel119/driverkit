...
func (v *ubuntu) SearchLocalKernelFilepath(cfg Config, kr kernelrelease.KernelRelease) ([]string, error) {

	packageNamePatterns := getUbuntuPackageNamePatterns(kr, cfg.Build.KernelVersion)

	localKernelFilePath, err := GetLocalKernelFiles(cfg.LocalKernelDir, v.Name(), packageNamePatterns)
	if err != nil {
		return nil, err
	}

	//ubuntu need *_all.deb and *_{arch}.deb
	archPkgPath := ""
	archPkgPattern := kr.Architecture.String() + ".deb"
	allPkgPath := ""
	allPkgPattern := "all.deb"

	for _, path := range localKernelFilePath {
		if strings.HasSuffix(path, archPkgPattern) {
			archPkgPath = path
		} else if strings.HasSuffix(path, allPkgPattern) {
			allPkgPath = path
		}
	}

	if archPkgPath == "" || allPkgPath == "" {
		return nil, fmt.Errorf("failed to found local kernel file: %v, both _all.deb and _%s.deb are required", localKernelFilePath, kr.Architecture.String())
	}

	return []string{archPkgPath, allPkgPath}, nil
}
...
func getUbuntuPackageNamePatterns(kr kernelrelease.KernelRelease, kernelVersion string) []string {
...
func fetchUbuntuKernelURL(baseURL string, kr kernelrelease.KernelRelease, kernelVersion string) ([]string, error) {
	...
	// parse the extra number and flavor for the kernelrelease extraversion
	_, ubuntuFlavor := parseUbuntuExtraVersion(kr.Extraversion)
	...
	// piece together all possible naming patterns for packages
	// 2 urls should resolve: an _{arch}.deb package and an _all.deb package
	packageNamePatterns := getUbuntuPackageNamePatterns(kr, kernelVersion)
