...
func (b *Build) LoadImages() {
	...
	for _, repo := range b.BuilderRepos {
		if IsOnlineMode() {
			...
			if err != nil {
				logger.Warnf("image search error from repo %s: %s\n", repo, err.Error())
				continue
			}
			for i := range imgs {
				imgNames = append(imgNames, imgs[i].Name)
			}
		} else {
			imgs, err := cli.ImageList(context.Background(), types.ImageListOptions{All: true})
			if err != nil {
				logger.Warnf("list local image error: %s\n", err.Error())
				continue
			}
			for i := range imgs {
				if len(imgs[i].RepoTags) > 0 {
					imgNames = append(imgNames, strings.Split(imgs[i].RepoTags[0], ":")[0])
				}
			}
		}
		...
		for _, imgName := range imgNames {
			...
			for regIdx, reg := range regs {
				match := reg.FindStringSubmatch(imgName)
				if len(match) == 0 {
					continue
				}
				...
				if len(gccVers) == 0 {
					logger.Debug("Malformed image name: ", imgName, len(match))
					continue
				}
